...
def detect_fall(landmarks_2d, bbox):
    ...
    # 2. 脊柱长度（肩髋距离）：弯腰时脊柱仍然伸展，摔倒时脊柱缩短
    spine_dist = ((dx)**2 + (dy)**2) ** 0.5
    left, top, width, height = bbox
    spine_ratio = spine_dist / max(height, 1)  # 脊柱占bbox高度的比例
    spine_maintained = spine_ratio > 0.35  # 脊柱仍然伸展 → 弯腰而非摔倒

    # 3. bbox宽高比（摔倒时宽>高）
    aspect_ratio = width / max(height, 1)

    # 4. 头部相对位置（头部在bbox下半部分）
    head_low = False
    if nose[2] > 0.3:
    ...
    hip_y = (lh[1] + rh[1]) / 2
    body_flat = abs(shoulder_y - hip_y) < height * 0.15
    ...
    elif aspect_ratio > 2.0 and head_low and spine_ratio < 0.25:
        # 极端宽高比+头部低位+脊柱极短（完全躺平）
        is_falling = True
        reasons.append(f"宽高比{aspect_ratio:.1f}+躺平")
